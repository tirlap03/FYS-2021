...
def k_means_algorithm(data, K, max_iters=100, threshold=0.001):
    ...
    while not converged and iters < max_iters:
        print(f"\n\n This is iteration number {iters}\n")
        ...
        # new_clusters = assign_data_to_cluster(data, new_centroids)
...
#     # Remove denormalization - just use original data for visualization
#     data_visual = data
#     centroids_visual = centroids
    
#     fig, axes = plt.subplots(K, 6, figsize=(12, 2*K))
    
#     for i in range(K):
#         image_index = np.where(clusters == i)[0]
#         cluster_images = data_visual[clusters == i]             
#         print(f"Processing cluster {i} with {len(cluster_images)} images")
        
#         # Still use normalized data for distance calculations
...
def visualize_clusters(data, centroids, clusters, K):
    ...
    plt.tight_layout()
    plt.savefig("Results/cluster_results_with_average.png")
    plt.show()
...
def visualize_sklearn_clusters(data, sklearn_kmeans, K):
    ...
    plt.tight_layout()
    plt.savefig("Results/sklearn_cluster_result_with_average.png")
    plt.show()
